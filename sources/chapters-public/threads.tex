...
\end{tabbing}
The result depends on the sequence in which the processors read and
...
temporary \indexterm{lock} on certain memory areas. 
Locks can for instance be implemented through the 
\indexterm{semaphore}
...
system level, so they are relatively slow. Since we hope that
