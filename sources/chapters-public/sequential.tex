...
aspects in common. On a very high level of abstraction,
many architectures can be described
as \indexterm{von Neumann architectures}. This describes a
design with an undivided memory that stores both program and
data (`stored program'), and a processing unit that executes the
...
\Level 0 {Modern floating point units}
\label{sec:fp}
...
Another way to increase performance is to have a `fused multiply-add'
...
Intel Pentium4&2 add or 2 mul&2\\ % simd
...
\input chapters-public/pipeline
...
\Level 1 {Pipelining beyond arithmetic: instruction-level parallelism}
...
\indexterm{instruction pipeline}\index{pipeline!instruction} as soon
...
This concept is also known as \indexacf{ILP}, and it is facilitated by various mechanisms:
\begin{itemize}
...
pipeline can stall. CPUs have taken to \indexterm{speculative
  execution} for instance, by always assuming that the test will turn
out true. If the code then takes the other branch (this is called a
\indexterm{branch misprediction}), the pipeline has to be cleared and
...
so-called \indextermbus{memory}{wall}: the memory is too slow to load data
...
this case for the \indextermbus{AMD}{Opteron} chip: the
...
\item Main memory access has a latency of more than 100 cycles, and a
  ...
  some bandwidth is spent on maintaining \indextermbus{cache}{coherence} (see
...
\begin{wrapfigure}{r}{3in}
  \includegraphics[scale=.07]{graphics-public/stride-1}
...
\begin{wrapfigure}{r}{3in}
  \includegraphics[scale=.07]{graphics-public/stride-3}
...
performance. To force allocated space to be \emph{aligned} with a
\indextermbus{cacheline}{boundary aligment}, you can do the following:
\begin{verbatim}
double *a;
...
\end{verbatim}
This code allocates a block of memory, and, if necessary, shfits it
right to have a starting address that is a multiple of~8. This sort of
alignment can sometimes be forced by compiler options.
...
\begin{figure}
  \begin{quote}
  \includegraphics[scale=.5]{graphics-public/ScCC-1}
  \includegraphics[scale=.4]{graphics-public/ScCC-2}
  \end{quote}
...
\label{sec:coherence}\index{cache!coherence|(}
...
longer an accurate copy of its counterpart,
so it needs to reload that item. This is known as \emph{maintaining
...
memory locations an \indexterm{atomic operation}; more about this in section~\ref{sec:shared-lock}.)
...
In this section we will take a look at \indexterm{data reuse}: are
...
giving a data reuse of~$1/3$. The \indexterm{axpy} (for
...
\begin{figure}[ht]
  \includegraphics[scale=.1]{graphics-public/ijk-mult}
...
\input chapters-public/memorycoding

\Level 0 {Power consumption}
\label{sec:power}
\index{power!consumption|(}
\input chapters-public/power
\index{power!consumption|)}
