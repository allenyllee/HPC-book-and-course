...
operating on only a single bit at a time. Along these lines, \emph{ICL}\index{ICL!DAP}
had the 4096 processor DAP~\cite{DAP:79a} in the 1980s, and \emph{Goodyear}\index{Goodyear!MPP}
...
the \indexterm{Columbia} computer at NASA consists of twenty 
nodes,
connected by a switch, where each \indexterm{node} is an NUMA architecture of 512
processors.
...
differ from sequential programs in \indexterm{directives} that are hidden in
