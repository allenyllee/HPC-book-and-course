...
\Level 1 {Finite difference approximation: Euler explicit and implicit methods}
\label{sec:fd-ode}
...
\indexterm{Taylor series} expansion\footnote{See appendix~\ref{app:taylor} 
...
previous point in space\footnote{This leads to a condition limiting
  ...
  \indexterm{Courant-Friedrichs-Lewy
    condition}~\url{http://en.wikipedia.org/wiki/CourantÐFriedrichsÐLewy_condition}.
...
every other point, no matter how little. The \indexterm{Poisson
  equation} (section~\ref{sec:poisson}) is the standard example of
this type.
\end{itemize}
...
applying the \indextermdef{difference stencil}
\[
\begin{matrix}
...
\begin{wrapfigure}{r}{2.5in}
  ...
  \includegraphics[scale=.12]{graphics-public/euler-forwardpdf}
...
\begin{wrapfigure}{r}{2.5in}
  \leavevmode\kern\unitindent 
  \includegraphics[scale=.20]{graphics-public/euler-backwardpdf}
