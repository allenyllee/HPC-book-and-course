...
We can write this more compactly by omiting the $x_i$ coefficients. Write
\[ 
\begin{pmatrix}
  6&-2&2\\ 12&-8&6\\ 3&-13&3
\end{pmatrix}
...
\indexterm{pivoting}.  It is not hard to show\footnote{And you can
...
A further point to mention here is that both the factorization
...
As a simple example, consider the matrix-vector product $y=Ax$, expressed in
block terms.
\[ 
\begin{pmatrix}
  Y_1\\ \vdots\\ Y_M
...
\input chapters-public/sparse
\index{sparse!linear algebra|)}

\Level 0 {Iterative methods}
\label{sec:iterative}
\index{iterative methods|(}
\index{Krylov methods|(}
\input chapters-public/stationary
\input chapters-public/cg
\index{iterative methods|)}
\index{Krylov methods|)}
