...
cache. Both caches have 12 elements, but addressed diferently.
...
\begin{figure}[ht]
  \includegraphics[scale=.3]{stampede-node}
  \caption{A two-socket design with co-processor: one node of the
...
which shows the two-socket node of the \indexterm{Stampede}
supercomputer.
...
It turns out that in scientific computing it is often the case that data
