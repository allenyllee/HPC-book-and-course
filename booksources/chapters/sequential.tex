...
\begin{remark}
  ...
  mappying virtual addresses to \indextermbus{memory}{pages}.
\end{remark}
...
cache. Both caches have 12 elements, but these are used diferently.
...
instance the \indextermbus{Intel}{Knight's Landing} has Gigabyte
pages.)
...
to it. This derives from the fact that cluster \indexterm{node} can
have more than one \indextermdef{socket}, that is, processor chip.
\begin{figure}[ht]
  \hbox{
    \includegraphics[scale=.15]{ranger-node}
    \includegraphics[scale=.15]{stampede-node}
    }
...
which shows the four-socket node of the \indexterm{Ranger}
supercomputer (no longer in production) and the two-socket node of the \indexterm{Stampede}
...
This is an example of \indexac{NUMA} design: for a process running on
some core, the memory attached to its socket is slightly faster to
access than the memory attached another socket.

One result of this is the \indextermdef{first-touch} phenomenon.
Dynamically allocated memory is not actually allocated until it's
first written. Consider now the following OpenMP
...
memory of the socket of the master thread. In the subsequenty parallel
...
CPUs\footnote{Low level memory access can be controlled by the
