...
\index{matrix!matrix product!parallel|(}
...
\item With $N^3$ operations on $O(N^2)$ elements, there is
  ...
  \index{matrix!matrix product!data reuse in}
  %
...
\item The distributed memory version of the matrix-matrix product is
  ...
  row, and elements of $B$~through a processor columns. We discuss the
  `Cannon algorithm' below.
\end{itemize}

\input chapters/goto

\Level 1 {Distributed memory matrix-matrix product}
...
% (see tutorial section~\ref{tut:petsc})
...
even
%
\emph{load distribution}\index{load distribution!of iteration space}.
%
