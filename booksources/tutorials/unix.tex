...
including \indexterm{Cygwin} on Windows.
...
\item As mentioned just now, there are different shells, and they do differ
  ...
  reasons is to be preferred over the \indexterm{csh} or \indexterm{tcsh} shell.
  Other shells are the \indexterm{ksh} and \indexterm{zsh}.
\end{itemize}
...
\practical{If the \n{ls} command shows that there are files, do \n{ls
...
\Level 0 {Shell environent variables}
\label{tut:shellvars}
...
For that you need the \indextermtt{export} command.
...
You can invoke a shell script with arguments:
\begin{verbatim}
./my_script -a file1 -t -x file2
\end{verbatim}
where you define how many arguments and what they mean.
...
user' and do things with their priviliges.
...
The streaming editor \n{sed} is like an editor by remote control,
...
The \n{awk} utility also operates on each line, but it can be
