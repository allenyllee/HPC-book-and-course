...
\item Within Linux there are various \indextermbus{Linux}{distributions}
  ...
  about them.
\item As mentioned just now, there are different shells, and they do
  differ considerably. Here you will learn the \indextermdef{bash}
  shell, which is an improved version of the old \indexterm{sh} shell.
   For a variety of reasons, \n{bash} is to be preferred over the
  \indexterm{csh} or \indexterm{tcsh} shell.  Other shells are the
  \indexterm{ksh} and \indexterm{zsh}, which is itself an improvement
  over the \n{bash} shell.
\end{itemize}
...
\begin{tabular}{|l|l|}
  ...
  command&function\\
  \hline
  \tt ls & list files or directories\\
  \tt touch & create new/empty file or update existing file\\
  \verb+cat > filename+ & enter text into file\\
  \tt cp & copy files\\
  \tt mv & rename files\\
  \tt rm & remove files\\
  \tt file & report the type of file\\
  \tt cat filename & display file\\
  \tt head,tail & display part of a file\\
  \tt less,more & incrementally display a file\\
  \hline
\end{tabular}

\Level 2 {\tt ls}

Without any argument, the \indextermunix{ls} command gives you a
listing of files that are in your present location.

\practical
{Type \indextermunix{ls}. Does anything show up?}
...
\practical{If the \indextermunix{ls} command shows that there are files, do \n{ls
    name} on one of those. By using an option, for instance \n{ls -s
    name} you can get more name.}{}{If you specify a name of a
  non-existing file, you'll get an error message.}

\Level 2 {\tt cat}

The \indextermunix{cat} command is often used to display files, but it can also be
...
{Type \n{cat > newfilename} (where you can pick any
  ...
  Now use \indextermunix{cat} to view the contents of that file: \n{cat newfilename}.}
{In the first use of \indextermunix{cat}, text was con\n{cat}enated from the
...
The \indextermunix{ls} command can give you all sorts of information.

\practical {Read the man page of the \indextermunix{ls} command: \n{man ls}. Find
  out the size and the time/date date of the last change to some files, for instance the file
  you just created.}
{Did you find the \n{ls -s} and \n{ls -l}
...
{The \indextermunix{man} command
  puts you in a mode where you can view long text documents. This
  viewer is common on Unix systems (it is available as the \indextermunix{more} or
  \indextermunix{less} system command), so memorize the following ways of navigating: Use
...
\begin{remark}
  ...
  sort. The \indextermunix{stat} command gives all of them.
\end{remark}
\begin{remark}
  ...
  command, \indextermunix{man}~\n{-k keyword} can help you find it.
\end{remark}

\Level 2 {\tt touch}

The \indextermunix{touch} command creates an empty file, or updates the timestamp of
a file if it already exists. Use \n{ls -l} to confirm this behaviour.

\Level 2 {\tt cp, mv, rm}

The \indextermunix{cp} can be used for copying a file (or directories,
see below): \n{cp file1 file2} makes a copy of \n{file1} and names it
\n{file2}.

\practical{Use \n{cp file1 file2} to copy a file. Confirm that the two files have
  the same contents. If you change the original, does anything happen
  to the copy?}{You should see that the copy does not change if the
  original changes or is deleted.}{If \n{file2} already exists, you
  will get an error message.}

A file can be renamed with \indextermunix{mv}, for `move'.

\practical{Rename a file. What happens if the target name already
  exists?}{}{}
...
\n{more}, and
\n{wc} using that file.}
{\indextermunix{head} displays the first couple of lines of a file, \indextermunix{tail} the
  last, and \indextermunix{more} uses the same viewer that is used for man
  pages. Read the man pages for these commands and experiment with
  increasing and decreasing the amount of output. 
  The \indextermunix{wc} (`word count') command reports the number of words,
  characters, and lines in a file.}{}

Another useful command is \indextermunix{file}: it tells you what
type of file you are dealing with.

\practical{Do \n{file foo} for various `foo': a text file, a
  directory, or the \n{/bin/ls} command.}{Some of the information may
  not be intelligible to you, but the words to look out for are
  `text', `directory', or `executable'.}{}
...
\begin{tabular}{|l|l|}
  ...
  command&function\\
  \hline
  \tt ls & list the contents of directories\\
  \tt mkdir & make new directory\\
  \tt cd & change directory\\
  \tt pwd & display present working directory\\
  \hline
\end{tabular}
...
is \indextermunix{pwd}. Your home directory is your working directory immediately
...
\practical{Make a new directory with \indextermunix{mkdir}~\n{newdir} and view the
...
working directory, is \indextermunix{cd} (`change directory'). It can be used in
the following ways:
\begin{itemize}
...
can alter. You can set permissions by the \indextermunix{chmod} command:
\begin{verbatim}
...
\begin{remark}
  ...
  \indexterm{setuid} bit declares that the program should run with the
  ...
  \indextermbus{root}{priviliges} are needed. Thanks to the setuid
...
The \indextermunix{grep} command can be used to search for a text expression in a
file.
...
basis. The stream editor \indextermunix{sed} is one example. If you have used the
...
Another tool for editing lines is \indextermunix{cut}, which will cut up a line
...
\practical{What you may think of as `Unix commands' are often just
  executable files in a system directory. Do \indextermunix{which}~\n{ls}, and do an
  \n{ls -l} on the result}
...
details see below) \indextermunixdef{PATH}.
...
The command \indextermunix{ps} can tell you everything that is currently running.
...
full list of all variables known to the shell by typing \indextermunixdef{env}. 
...
\practical{Type \n{a=5} on the commandline. This defines a variable
  \n{a}; check on its value by using the \n{echo} command.}{The shell
  will respond by typing the value~\n{5}.}{Beware not to have space
...
For that you need the \indextermunix{export} command.
...
\indextermunixdef{if}, and it can be written over several
lines:
\begin{verbatim}
if [ $PATH = "" ] ; then
...
\end{verbatim}
(The backquote is explain in section~\ref{tut:unix-backquote}.)
...
\begin{itemize}
\item \indextermunix{if} and \indextermunix{elif} are followed by a
  conditional, followed by a semicolon.
\item The brackets of the conditional need to have spaces surrounding
  them.
\item There is no semicolon after \indextermunix{then} of \indextermunix{else}.
\end{itemize}

\begin{exercise}
  Bash conditionals have an \indextermunix{elif} keyword. Still you can write
  the sequence \n{else if}, as in:
\begin{verbatim}
if [ something ] ; then
  foo
...
The above construct easily loops over words, such as the output of
...
contained in \indextermtt{PATH}; see section~\ref{sec:PATH}.
...
The standard way to parse argument is using the \indextermunixdef{shift}
command, which pops the first argment off the list of arguments.
...
The variables \verb+$@+ and \verb+$*+ have a different behaviour with
...
\indextermbus{root}{priviliges} only when strictly needed.
%
(The root account is also known as the \indextermsubdef{super}{user}.)
%
If you have root priviliges, you can also use that to `become another
user' and do things with their priviliges, with the
\indextermunixdef{sudo} (`superuser do') command.
...
\Level 0 {Other systems: {\tt ssh} and {\tt scp}}
...
the `secure shell' command~\indextermunixdef{ssh}, a~more secure variant
of the old `remote shell' command~\indextermunixdef{rsh}:
\begin{verbatim}
...
`secure copy' \indextermunixdef{scp}, a~secure variant of `remote
copy'~\indextermunixdef{rcp}. The \n{scp} command is much like \n{cp} in syntax,
...
The streaming editor \indextermunixdef{sed} is like an editor by remote control,
doing simple line edits with a commandline interface. Most of the time
you will use \n{sed} as follows:
\begin{verbatim}
cat somefile | sed 's/abc/def/:g' > newfile
\end{verbatim}
(The use of \n{cat} here is not strictly necessary.)
The \n{s/abc/def/} part has the effect of replacing \n{abc} by \n{def}
in every line; the \n{:g} modifier applies it to every instance in
...
The \indextermunixdef{awk} utility also operates on each line, but it can be
...
\end{verbatim}
  ...
  \indextermunix{diff} command to see if the submitted file is
  ...
  inserted newlines. This can not be done with \indextermunix{diff}, but you could
  try \indextermunix{tr} to remove the newlines.
\end{exercise}
