...
\begin{figure}[ht]
  \hbox{$\vcenter{\hbox{\includegraphics[scale=.1]{graphics/graph1}}}$
    $
    \begin{cases}
      V=\{1,2,3,4,5,6\}\\
...
\begin{figure}[ht]
  \hbox{$\vcenter{\hbox{\includegraphics[scale=.1]{graphics/graph2a}}}$
    A graph}
  \hbox{$\vcenter{\hbox{\includegraphics[scale=.1]{graphics/graph2b}}}$
    Two paths from 1 to 6; $\langle 1,4,6\rangle$ is the shorter}
  \hbox{$\vcenter{\hbox{\includegraphics[scale=.1]{graphics/graph2c}}}$
...
applying a \indextermsub{permutation}{matrix}.
\begin{definition}
...
\Level 1 {Graph operations in linear algebra}
...
This left matrix-vector product has a neat application: \indextermbus{Markov}{chains}.
...
\Level 2 {General matrix-vector product}
...
We start by remarking that the following theorems may not seem
intuitive, but they closely resemble some simple facts
about \acp{PDE}. 
...
connected pieces: one of nodes where the eigenvector is positive, and
