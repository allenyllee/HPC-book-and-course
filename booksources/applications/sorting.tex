...
\Level 0 {Sorting through MapReduce}
\label{sec:terasort}
\index{Terasort|(}
...
\item The mapper processes then split the data into one bin for
  each reducer.
...
\begin{itemize}
\item The local binning, which takes time~$O(b)$, and
\item The local sort, for which is we can an optimal complexity
  of~$b\log b$.
\end{itemize}
...
Comparing this algorithm to sorting networks like bitonic sort, above,
