program quiz
  ...
  if(rank == prev_rank)then 
    call MPI_SEND(local_value,1,MPI_INTEGER,next_rank,tag1,MPI_COMM_WORLD,ierr)  
    call MPI_RECV(prev_value ,1,MPI_INTEGER,prev_rank,tag1,MPI_COMM_WORLD,stats(:,1),ierr)
  else if(rank == next_rank)then
    call MPI_SEND(local_value,1,MPI_INTEGER,prev_rank,tag2,MPI_COMM_WORLD,ierr)  
    call MPI_RECV(next_value ,1,MPI_INTEGER,next_rank,tag2,MPI_COMM_WORLD,stats(:,2),ierr)
  end if 
